  ...
  async checkProductHasNotBeenOrdered (productId) {
   ...
   async searchProducts(query) {
    try {
      ...
      const products = await RestaurantMongoose.aggregate([
        {
          $unwind: "$products"
        },
        {
          $match: {
            $or: [
              { "products.name": { $regex: regexQuery } },
              { "products.description": { $regex: regexQuery } }
            ]
          }
        },
        {
          $project: {
            ...
            productCategory: "$products._productCategoryId",
